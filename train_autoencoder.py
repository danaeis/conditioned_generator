...
def visualize_batch(batch, output, epoch, batch_idx, save_dir='visualizations'):
    ...
    input_phase_condition = batch['phase_conditions'][input_phase].cpu().numpy()
    ...
    # Plot input phase condition
    plot_slice(axes[0, 1], input_phase_condition, 'Input Phase Condition')
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(tqdm(dataloader, desc=f'Epoch {epoch+1}/{num_epochs}')):
            ...
            for target_phase in dataset.phases:
                ...
                # Forward pass with both input and output phase conditions
                optimizer.zero_grad()
                
                # Ensure inputs require gradients
                input_volume.requires_grad_(True)
                phase_conditions[input_phase].requires_grad_(True)
                phase_conditions[target_phase].requires_grad_(True)
                
                reconstructed = model(
                    x=input_volume,
                    input_phase_condition=phase_conditions[input_phase],
                    output_phase_condition=phase_conditions[target_phase]
                )
                ...
                if batch_idx % 10 == 0:
                    visualize_batch(batch, reconstructed, epoch, batch_idx, save_dir=os.path.join(output_dir, 'visualizations'))
