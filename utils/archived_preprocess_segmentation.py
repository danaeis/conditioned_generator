...
from archived_volume_utils import crop_center, resample_to_target, resample_spacing, detect_abdomen_bbox
...
# Add the path to the UNETR module
unetr_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "../research-contributions/UNETR/BTCV/networks"))
sys.path.append(unetr_path)
# print("UNETR path added:", unetr_path)
# print("sys.path:", sys.path)
# Now you can import UNETR
from unetr import UNETR


def interpolate_position_embeddings(checkpoint, target_num_patches=1728, hidden_size=768):
    pos_embed = checkpoint["vit.patch_embedding.position_embeddings"]  # [1, 216, 768]
    old_shape = int(round((pos_embed.shape[1]) ** (1/3)))  # 6
    pos_embed = pos_embed.reshape(1, old_shape, old_shape, old_shape, hidden_size).permute(0, 4, 1, 2, 3)  # [1, 768, D, H, W]

    new_shape = int(round(target_num_patches ** (1/3)))  # 12
    pos_embed_resized = torch.nn.functional.interpolate(
        pos_embed, size=(new_shape, new_shape, new_shape), mode='trilinear', align_corners=False
    )
    pos_embed_resized = pos_embed_resized.permute(0, 2, 3, 4, 1).reshape(1, -1, hidden_size)  # [1, 1728, 768]

    checkpoint["vit.patch_embedding.position_embeddings"] = pos_embed_resized
    return checkpoint
...
def resample_to_k_slices(volume, original_spacing, target_slices=250):
    size = volume.GetSize()
...
checkpoint = torch.load("unetr_model/UNETR_model_best_acc.pth")
checkpoint = interpolate_position_embeddings(checkpoint)
...
for study_uid, series_uid, volume in tqdm(dicom_series):
    ...
    vol_250 = resample_to_k_slices(volume, spacing, 192)
